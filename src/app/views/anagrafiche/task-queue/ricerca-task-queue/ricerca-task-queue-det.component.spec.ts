...
describe('RicercaTaskQueueComponent', () => {
  ...
  const mockTaskService = {
    getTipoTask: () =>  of([{codice: 1 , descrizione: 'ECC_TR_RET'}]),
  ...
  it('if data is ecc_tr_ret should show buttons ', () => {
    const model: ActionRenderModel = component.buildActionRenderModel(dataEccTransc);
    expect(model.detailButton).not.toBeNull();
  });

  it('if data is ecc_ra_ret should show buttons ', () => {
    const model: ActionRenderModel = component.buildActionRenderModel(dataEccRagg);
    expect(model.detailButton).not.toBeNull();
  });
