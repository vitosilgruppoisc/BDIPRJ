...
describe('RicercaEccTranscodingIngrossoComponent', () => {
  ...
  it('should not search if not valid', () => {
    component.bicConto.setValue('A');
    component.submit(el, true);
    const req = httpTestingController.expectNone('/eccTranscodingIngrosso');
  });

  it('should search if valid', () => {
    component.submit(el, true);
    const req = httpTestingController.expectOne('/eccTranscodingIngrosso?');
